...
Tensor rebuild_padding(Tensor input, Tensor sequence_id_offset, Tensor attention_mask, int64_t int8_mode) {
  ...
  switch (_st) {
    case at::ScalarType::Float:
      return rebuild_padding_impl<float>(input, sequence_id_offset, attention_mask, (int)int8_mode);
    case at::ScalarType::Half:
      return rebuild_padding_impl<half>(input, sequence_id_offset, attention_mask, (int)int8_mode);
    default:
      throw std::runtime_error("Wrong Tensor type.");
  }
}
